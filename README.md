...
Herdr's plugin marketplace is unmoderated: any repository tagged `herdr-plugin` appears in it, and there were **1,312** of them when this list was last audited. This list is the opposite — a short set of things with a track record.
...
**How the plugin entries were picked:** every repository in Herdr's [marketplace](https://herdr.dev/plugins/) with 25+ stars — 73 of the 1,312 — ranked and checked one by one. What is missing is missing on purpose: personal dotfiles repositories, tools where Herdr is one interchangeable backend among several rather than the point, and otherwise-qualifying projects that ship no license, including `pi-extensible-workflows` (⭐ 228), `herdr-board` (⭐ 149), and `herdr-tab-smart-rename` (⭐ 77). They are recorded in [`data/sources.json`](data/sources.json) and go back under review the moment a license appears. `herdrm` (⭐ 709), a native macOS agent console, adopted one on 2026-09-10, but it is the noncommercial PolyForm license, so it is tracked rather than headed here until noncommercial terms are confirmed to clear the bar.
...
- **2026-06-30 · Growth** — **40,211** stars on 2026-09-22, up from 38,297 on 2026-09-14 and roughly 15k in early July, when it hit #1 on GitHub Trending; the 0.9 announcement put downloads above **700,000**. The plugin marketplace held **1,312** repositories on the same date, up from 1,144; 73 of them clear this list's 25-star bar, up from 66. [Repository](https://github.com/herdrdev/herdr) · [Marketplace](https://herdr.dev/plugins/) · [Downloads](https://herdr.dev/blog/connecting-the-machines/)
